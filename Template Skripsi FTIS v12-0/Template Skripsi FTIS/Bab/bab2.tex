...
\begin{enumerate}
	...
	\end{figure}
			...
			Pada bidang kesehatan, CA juga sering digunakan untuk pemodelan penyebaran penyakit. Biasanya masalah penyebaran penyakit dimodelkan dengan CA dua dimensi dan menggunakan aturan Game of Life dari Conway. Contoh aplikasi yang diterapkan di dunia nyata yaitu simulasi infeksi virus influenza A menggunakn cellular automaton. Pada penelitian ini cellular automata yang digunakan adalah CA dua dimensi. Parameter-parameter masukan yang digunakan adalah sebagai berikut
			\begin{enumerate}
				\item \textit{gridWidth} dan \textit{gridHeight} yang berfungsi mendefinisikan lebar dan tinggi dari \textit{grid} yang digunakan.
				\item \textit{cellLifespan} merupakan parameter yang menyimpan lama waktu hidup dari sel epitel sehat.
				\item \textit{infectLifespan} merupakan parameter yang menyimpan lama waktu hidup dari sel epitel yang sakit.
				\item \textit{divisionTime} merupakan durasi pembelahan sel epitel yang sehat.
				\item \textit{infectInit} merupakan proporsi inisialisasi sel epitel yang terinfeksi dari keseluruhan sel epitel yang ada.
				\item \textit{infectRate} merupakan banyaknya tetangga yang dapat ditulari sel epitel yang berpenyakit dalam waktu satu jam.
				\item \textit{expressDelay} merupakan lama waktu tunggu perubahan status sel epitel \textit{infected} menjadi \textit{expressing}
				\item \textit{infectDelay} merupakan lama waktu tunggu perubahan status sel epitel \textit{infected} menjadi \textit{infectious}
				\item \textit{baseImmCell} merupakan proporsi minimal sel imun virgin dari keseluruhan sel epitel.
				\item \textit{immLifespan} adalah jangka waktu hidup sel imun.
				\item \textit{recruitChance} adalah kemungkinan penambahan sel imun \textit{mature} ke dalam \textit{grid}.
				\item \textit{recruitDelay} adalah jeda waktu pengenalan sel imun terhadap sel epitel yang mengandung virus sampai sel imun \textit{mature} ditambahkan ke dalam \textit{grid}.
				\item \textit{flowrate} adalah perkiraan kecepatan sel imun berpindah dari satu sel epitel ke epitel lain dalam waktu satu jam. \cite{referensiCA1}
			\end{enumerate}
			Aturan dalam penelitian ini ada dua jenis aturan yaitu aturan transisi dalam kelas \textit{EpithelialCell} dan kelas \textit{ImmuneCell}.
...
\begin{itemize}
	...
	\item $N = {N_{1}, ..., N_{k}}$ adalah himpunan berhingga ketetanggaan dimana setiap $N_{i}$ didefinisikan sebagai $N_{i}:E x E \rightarrow $
\end{itemize}
...
Adjacency Matrix merupakan representasi matrix $N*N$ yang menyatakan hubungan antar simpul dalam suatu graf. Kolom dan baris menyatakan simpul-simpul, sedangkan nilai entri dari matrix menyatakan hubungan antar simpul. Contoh implementasi adjacency matrix yaitu sebagai berikut :
